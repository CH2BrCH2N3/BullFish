...
dist5 = [0]
speed = [0]
for i in range(step + 1, l, step):
    dist5.append(Decimal.sqrt((com[i][0] - com[i - step][0]) ** 2 + (com[i][1] - com[i - step][1]) ** 2))
for i in range(1, l):
...
total_distance = sum(dist5)

max_distance_02s = sum([dist5[i] for i in range(1, info[0][1] // 5 + 1)])
distance_02s = max_distance_02s
for i in range(info[0][1] // 5 + 1, l // step):
    distance_02s += (dist5[i] - dist5[i - info[0][1] // 5])
    if distance_02s > max_distance_02s:
        max_distance_02s = distance_02s

max_distance_1s = sum([dist5[i] for i in range(1, info[0][1] + 1)])
distance_1s = max_distance_1s
for i in range(info[0][1] + 1, l // step):
    distance_1s += (dist5[i] - dist5[i - info[0][1]])
...
with open('converted.csv', 'w') as file:
    header = ['Frame', 'x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'x4', 'y4', 'x5', 'y5', 'x6', 'y6', 'x7', 'y7', 'Distance stdev', 'Spine length', 'Direction', 'Turn', 'Turn avg', 'Angle0', 'Angle0 change', 'Angle0 avg', 'Angle6', 'Angle6 avg', 'Angle6 change', 'Angle6 change avg', 'X', 'Y', 'Freeze', 'Speed', 'Acceleration']
    for word in header:
        file.write(str(word) + ', ')
    file.write('\n')
    ...
    for i in range(l):
        ...
        file.write(str(distance[0][i]) + ', ' + str(bl[0][i]) + ', ' + str(direction[i]) + ', ' + str(turn[i]) + ', ' + str(turn_avg[i]) + ', ' + str(angle[0][i]) + ', ' + str(angle0_change[i]) + ', ' + str(angle0avg[i]) + ', ' + str(angle[6][i]) + ', ' + str(angle6avg[i]) + ', ' + str(angle6_change[i]) + ', ' + str(angle6_change_avg[i]) + ', ' + str(com[i][0]) + ', ' + str(com[i][1]) + ', ' + str(freeze[i]) + ', ' + str(speed_avg[i]) + ', ' + str(accel_avg[i]) + '\n')
