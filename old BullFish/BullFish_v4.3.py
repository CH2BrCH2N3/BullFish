...
with open('kinematics_analysis.csv', 'w') as file:
    ...
    file.write(str(total_distance) + ', ' + str(total_distance / Decimal(l // fps - total_freeze_time)) + ', ' 
               + str(max(speed_avg)) + ', ' + str(max_distance_02s * 5) + ', ' + str(max_distance_1s) + ', '
...
if spine_analysis:
    ...
    with open('original_spine_turns_and_angles.csv', 'w') as file:
        header = ['Frame', 'Angle 1', 'Angle 2', 'Angle 3', 'Angle 4', 'Angle 5', 
                  ...
                  'headtotail_angle', 'headtotail_angle_avg', 'headtotail_angle_change', 
                  'total_abs_angle', 'total_abs_angle_avg', 'total_abs_angle_change']
        for word in header:
            file.write(str(word) + ', ')
        file.write('\n')
        for i in range(l):
            ...
            file.write(str(direction[i]) + ', ' + str(turn[i]) + ', ' + str(turn_avg[i]) + ', ' 
                       + str(angle[0][i]) + ', ' + str(headtotail_angle_avg[i]) + ', ' + str(headtotail_angle_change[i]) + ', ' 
                       + str(angle[6][i]) + ', ' + str(total_abs_angle_avg[i]) + ', ' + str(total_abs_angle_change[i]) + '\n')
    ...
    if spine_inversion or spine_othercorrections:
        ...
        with open('converted_spine__turns_and_angles.csv', 'w') as file:
            header = ['Frame', 'Angle 1', 'Angle 2', 'Angle 3', 'Angle 4', 'Angle 5', 
                      'Angle 1 change', 'Angle 2 change', 'Angle 3 change', 'Angle 4 change', 'Angle 5 change', 
                      'Direction', 'Turn', 'Turn avg', 
                      'headtotail_angle', 'headtotail_angle_change', 'headtotail_angle_avg', 
                      'total_abs_angle', 'total_abs_angle_avg', 'total_abs_angle_change']
            for word in header:
                file.write(str(word) + ', ')
            file.write('\n')
...
total_abs_angle_avg = np.array(total_abs_angle_avg)
peaks = find_peaks(total_abs_angle_avg, height = 0.0873, distance = 5)
...
with open('angle_analysis.csv', 'w') as file:    
