...
for file in os.listdir('.'):
    ...
    if filename.endswith("_p.avi"):
            ...
            if ret:
                ...
                if spine_analysis:
                    ...
                    for j in range(max_diameter):
                        current_area = sq_area(sframe, fish_contour[j][0][1], fish_contour[j][0][0], 4 + fish_area // 300)
        ...
        with open(path + '/' + videoname + '_raw.csv', 'w') as f:
            header = ['centroidX', 'centroidY', 'Distance', 'Distance over 1s', 'Freeze', 'Speed', 'Speed_avg']
            for word in header:
                f.write(str(word) + ', ')
            f.write('\n')
            for i in range(l):
                row = [cen[i][0], cen[i][1], cdist[i], cdist1[i], freeze[i], speed[i], speed_avg[i]]
                for cell in row:
                    f.write(str(cell) + ', ')
                f.write('\n')
        ...
        if spine_analysis:
            ...
            with open(path + '/' + videoname + '_raw_spine_points.csv', 'w') as f:
                f.write('spine(XY, XY, ...)' + '\n')
                for i in range(l):
                    row = []
                    for j in range(spine_len[i]):
