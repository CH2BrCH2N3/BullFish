...
from BullFish_pkg.plot import errors_correct, curve, step, agg1, agg2, agg3, DF, results_df
...
print('- [a]nalysis: After the representative coordinates have been generated by [t]racker, this program will')
    ...
    if program == 'v':
        ...
        for file in os.listdir('.'):
            ...
            try:
                ...
                threshold1s = np.zeros(l)
                if settings['find_s0']:
                    threshold2s = np.zeros(l)
        ...
        for file in os.listdir('.'):
            ...
            with open(path + '/' + videoname + '_spine.csv', 'r') as f:
                ...
                for i in range(l):
                    spine_temp = []
                    for j in range(spine_len):
                        spine_temp.append((float(spines[i][j * 2]) * ratio, float(spines[i][j * 2 + 1]) * ratio))
                    spines[i] = spine_temp
            ...
            stds = [stdev([pyth(spines[i - 1][j], spines[i][j]) for j in range(spine_len)]) for i in range(l)]
            ...
            trunk_amps = [[0 for j in range(spine_len - 2)] for i in range(l)]
            for i in range(l):
                if spines[i][spine_len - 1][0] == spines[i][spine_len - 2][0]:
                    for j in range(spine_len - 2):
                        trunk_amps[i][j] = abs(spines[i][j][0] - spines[i][spine_len - 1][0])
                else:
                    m = (spines[i][spine_len - 1][1] - spines[i][spine_len - 2][1]) / (spines[i][spine_len - 1][0] - spines[i][spine_len - 2][0])
                    c = spines[i][spine_len - 1][1] - m * spines[i][spine_len - 1][0]
                    for j in range(spine_len - 2):
                        trunk_amps[i][j] = abs(m * spines[i][j][0] - spines[i][j][1] + c) / sqrt(m ** 2 + 1)
                amps[i] = trunk_amps[i][0]
            
            fish_segs = [[0 for j in range(spine_len)] for i in range(l)]
            for i in range(l):
                for j in range(spine_len - 1):
                    fish_segs[i][j] = pyth(spines[i][j], spines[i][j + 1])
                fish_segs[i][spine_len - 1] = pyth(spines[i][spine_len - 1], heads[i])
